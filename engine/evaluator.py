...
class Evaluator(object):
    ...
    def sliding_eval_rgbX(self, img, X, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            ...
            if len(X.shape) == 2:
                X_scale = cv2.resize(X, None, fx=s, fy=s, interpolation=cv2.INTER_NEAREST)
            else:
                X_scale = cv2.resize(X, None, fx=s, fy=s, interpolation=cv2.INTER_LINEAR)

            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_process_rgbX(img_scale, X_scale, (ori_rows, ori_cols),
                                                        crop_size, stride_rate, device)
    ...
    def scale_process_rgbX(self, img, X, ori_shape, crop_size, stride_rate, device=None):
        new_rows, new_cols, c = img.shape
        long_size = new_cols if new_cols > new_rows else new_rows
        ...
        if new_cols <= crop_size[0] or new_rows <= crop_size[1]:
            ...
            X_pad, margin = pad_image_to_shape(X, crop_size, cv2.BORDER_CONSTANT, value=0)
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    s_y = e_y - crop_size[1]
                    img_sub = img_pad[s_y:e_y, s_x: e_x, :]
                    if len(X_pad.shape) == 2:
                        X_sub = X_pad[s_y:e_y, s_x: e_x]
                    else:
                        X_sub = X_pad[s_y:e_y, s_x: e_x,:]

                    input_data, input_X, tmargin = self.process_image_rgbX(img_sub, X_sub, crop_size)
                    temp_score = self.val_func_process_rgbX(input_data, input_X, device)
    ...
    def val_func_process_rgbX(self, input_data, input_X, device=None):
        input_data = np.ascontiguousarray(input_data[None, :, :, :], dtype=np.float32)
        input_data = torch.FloatTensor(input_data).cuda(device)
    
        input_X = np.ascontiguousarray(input_X[None, :, :, :], dtype=np.float32)
        input_X = torch.FloatTensor(input_X).cuda(device)
    
        with torch.cuda.device(input_data.get_device()):
            self.val_func.eval()
            self.val_func.to(input_data.get_device())
            with torch.no_grad():
                score = self.val_func(input_data, input_X)
                score = score[0]
                if self.is_flip:
                    input_data = input_data.flip(-1)
                    input_X = input_X.flip(-1)
                    score_flip = self.val_func(input_data, input_X)
                    score_flip = score_flip[0]
                    score += score_flip.flip(-1)
                score = torch.exp(score)
    ...
    def process_image_rgbX(self, img, X, crop_size=None):
        p_img = img
        p_X = X
        ...
        if crop_size is not None:
            ...
            p_X, _ = pad_image_to_shape(p_X, crop_size, cv2.BORDER_CONSTANT, value=0)
            p_img = p_img.transpose(2, 0, 1)
            if len(X.shape) == 2:
                p_X = p_X[np.newaxis, ...]
            else:
                p_X = p_X.transpose(2, 0, 1) # 3 H W
        
            return p_img, p_X, margin
