...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        img = data['data']
        label = data['label']
        polar = data['polar_img']
        name = data['fn']
        pred = self.sliding_eval_rgbX(img, polar, config.eval_crop_size, config.eval_stride_rate, device)
...
if __name__ == "__main__":
    ...
    val_pre = ValPre()
    dataset = ZJURGBP(data_setting, 'val', val_pre)

    with torch.no_grad():
        segmentor = SegEvaluator(dataset, config.num_classes, config.image_mean,
                                 config.image_std, config.polar_mean,
                                 config.polar_std, network,
